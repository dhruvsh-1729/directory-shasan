...
const ContactCard: React.FC<ContactCardProps> = ({ 
  ...
  const getLocationDisplay = () => {
    ...
    if (contact.country && !contact.country.toLowerCase().includes('india')) {
  ...
  return (
    <div
      className={`bg-white/90 backdrop-blur-sm rounded-xl shadow-sm border transition-all duration-300 hover:shadow-lg hover:border-blue-200/50 
        ...
        ${!contact.isMainContact ? 'ml-6 border-l-4 border-l-green-400' : ''}`}
    >
      ...
      <div className="p-4" onClick={onSelect}>
        <div className="flex items-center justify-between">
            ...
            <div className="ml-3 min-w-0 flex-1">
              <div className="flex items-center">
                ...
                {contact.duplicateGroup && (
                  <span className="ml-2 text-yellow-500" title="Potential duplicate">
                    <AlertTriangle className="h-4 w-4" />
                  </span>
                )}
                {hasValidationIssues && showValidation && (
                  <span className="ml-2 text-red-500" title="Validation issues">
                    <AlertTriangle className="h-4 w-4" />
                  </span>
                )}
              </div>
          ...
          <div className="hidden sm:flex items-center space-x-4 flex-shrink-0 mx-4">
            {primaryPhone && (
              <div className="flex items-center text-xs bg-gray-50 px-3 py-2 rounded-lg border">
                <Phone className="h-3 w-3 text-gray-400 mr-2" />
                <span className="font-mono text-gray-700">{primaryPhone.number}</span>
                {primaryPhone.country && primaryPhone.region && (
                  <span className="ml-2 text-sm">{getCountryFlag(primaryPhone.region)}</span>
                )}
            ...
            {primaryEmail && (
              <div className="hidden lg:flex items-center text-xs bg-gray-50 px-3 py-2 rounded-lg border max-w-[200px]">
                <Mail className="h-3 w-3 text-gray-400 mr-2 flex-shrink-0" />
                <span className="font-mono text-gray-700 truncate">{primaryEmail.address}</span>
          ...
          <div className="flex items-center space-x-2 flex-shrink-0 mr-2">
            {/* Contact Data Quality Indicators */}
            <div className="flex space-x-1 text-xs">
              <div className={`px-2 py-1 rounded-full flex items-center border ${
